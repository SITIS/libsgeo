			   ...
			   unsigned int size, unsigned int offset)
{
	...
	entry->tag        = (ExifTag)exif_get_short (d + offset + 0, data->priv->order);
	entry->format     = (ExifFormat)exif_get_short (d + offset + 2, data->priv->order);
			   ...
			   unsigned int offset)
{
	...
	if (s > 4) {
		...
		if (s & 1)
			ts++;
		t = (unsigned char*)exif_mem_realloc (data->priv->mem, *d, ts);
		if (!t) {
			       ...
			       unsigned int ds, ExifLong o, ExifLong s)
{
	...
	if (!(data->data = (unsigned char*)exif_data_alloc (data, s))) {
		EXIF_LOG_NO_MEMORY (data->priv->log, "ExifData", s);
		data->size = 0;
		return;
	}
			     ...
			     unsigned int ds, unsigned int offset, unsigned int recursion_depth)
{
	...
	for (i = 0; i < n; i++) {
		...
		tag = (ExifTag)exif_get_short (d + offset + 12 * i, data->priv->order);
		switch (tag) {
		case EXIF_TAG_EXIF_IFD_POINTER:
		case EXIF_TAG_GPS_INFO_IFD_POINTER:
		case EXIF_TAG_INTEROPERABILITY_IFD_POINTER:
		case EXIF_TAG_SGEO_IFD_POINTER:
		...
		case EXIF_TAG_JPEG_INTERCHANGE_FORMAT:
			...
			case EXIF_TAG_INTEROPERABILITY_IFD_POINTER:
				...
				break;
			case EXIF_TAG_SGEO_IFD_POINTER:
				CHECK_REC (EXIF_IFD_SGEO);
				exif_data_load_data_content (data, EXIF_IFD_SGEO, d, ds, o, recursion_depth + 1);
				break;
			case EXIF_TAG_JPEG_INTERCHANGE_FORMAT:
				thumbnail_offset = o;
		...
		default:
				...
				tag != SGEO_TAG_OBJECT_TABLE)
			{
				...
				if (!memcmp (d + offset + 12 * i, "\0\0\0\0", 4)) {
					exif_log (data->priv->log, EXIF_LOG_CODE_DEBUG, "ExifData",
						  "Skipping empty entry at position %u in '%d'.", i, 
						  ifd);
					break;
				}
				exif_log (data->priv->log, EXIF_LOG_CODE_DEBUG, "ExifData",
					  "Unknown tag 0x%04x (entry %u in '%d'). Please report this tag "
			     ...
			     unsigned int offset)
{
	...
	for (i = 0; i < (int)EXIF_IFD_COUNT; i++)
		if (ifd == data->ifd[(ExifIfd)i])
			break;
	if (i == EXIF_IFD_COUNT)
		return;	/* error */
	...
	*ds = ts;
	 ...
	 */
	exif_log (data->priv->log, EXIF_LOG_CODE_DEBUG, "ExifData",
		  "Saving %i entries (IFD '%i', offset: %i)...",
		  ifd->count, i, offset);
	...
	case EXIF_IFD_0:
		...
		if (data->ifd[EXIF_IFD_SGEO] != NULL && data->ifd[EXIF_IFD_SGEO]->count)
		{
			...
			exif_set_long  (*d + 6 + offset + 8, data->priv->order,
					*ds - 6);
			exif_data_save_data_content (data,
						     data->ifd[EXIF_IFD_SGEO], d, ds, *ds - 6);
			offset += 12;
		}
...
entry_set_byte_order (ExifEntry *e, void *data)
{
	...
	exif_array_set_byte_order (e->format, e->data, e->components, d->old, d->_new);
}
