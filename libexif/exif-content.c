...
exif_content_remove_entry (ExifContent *c, ExifEntry *e)
{
	...
	if (c->count > 1) {
		t = (ExifEntry **)exif_mem_realloc (c->priv->mem, c->entries,
...
exif_content_get_ifd (ExifContent *c)
{
	...
	return 
		...
		((c)->parent->ifd[EXIF_IFD_SGEO] == (c)) ? EXIF_IFD_SGEO :
		EXIF_IFD_COUNT;
}
...
remove_not_recorded (ExifEntry *e, void *UNUSED(data))
{
	...
	if (exif_tag_get_support_level_in_ifd (t, ifd, dt) ==
		...
		exif_log (c->priv->log, EXIF_LOG_CODE_DEBUG, "exif-content",
				"Tag 0x%04x is not recorded in IFD '%d' and has therefore been "
				"removed.", t, ifd);
	...
	for (i = 0; i < num; ++i) {
		...
		if (exif_tag_get_support_level_in_ifd (t, ifd, dt) ==
			...
			if (exif_content_get_entry (c, t))
				...
				continue;
			exif_log (c->priv->log, EXIF_LOG_CODE_DEBUG, "exif-content",
					"Tag '%s' is mandatory in IFD '%d' and has therefore been added.",
					exif_tag_get_name_in_ifd (t, ifd), ifd);
			e = exif_entry_new ();
