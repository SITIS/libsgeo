...
#define CLEANUP_REALLOC(p,s) { \
	unsigned char *cleanup_ptr = (unsigned char *)realloc((p),(s)); \
		...
		*ds += 2;
		...
		case JPEG_MARKER_APP1:
			...
			*ds += eds;
			...
			break;
		default:
			CLEANUP_REALLOC ((*d), sizeof (char) * (*ds + s.content._generic.size + 2));
			(*d)[*ds + 0] = (s.content._generic.size + 2) >> 8;
			(*d)[*ds + 1] = (s.content._generic.size + 2) >> 0;
			*ds += 2;
			memcpy (*d + *ds, s.content._generic.data,
				s.content._generic.size);
			*ds += s.content._generic.size;
	...
	for (o = 0; o < size;) {
		...
		default:
			...
			case JPEG_MARKER_APP1:
				...
				break;
			default:
				s->content._generic.data =
						(unsigned char*)malloc (sizeof (char) * len);
				if (!s->content._generic.data) return;
				s->content._generic.size = len;
				memcpy (s->content._generic.data, &d[o], len);
